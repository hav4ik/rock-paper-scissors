...
class GreenbergStrategy(BaseAtomicStrategy):
  def __init__(self):
    super().__init__()
    self.opponent_hist, self.my_hist = [], []
    self.act = None

  def __call__(self, history):
    rps_to_text = ('rock','paper','scissors')
    if len(history) > 0:
        self.act = MOVE_TO_NUM[history.our_moves[-1]]
        self.my_hist.append(rps_to_text[self.act])
        self.opponent_hist.append(rps_to_text[MOVE_TO_NUM[history.his_moves[-1]]])
    self.act = player(self.my_hist, self.opponent_hist)
    return NUM_TO_MOVE[self.act]
